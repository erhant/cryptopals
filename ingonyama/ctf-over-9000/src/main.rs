...
fn upload_solution(proof: &PowerProof) {
    ...
    // sends the proof as an array of bytes
    stream
        .write(&(proof_vec.len() as u64).to_be_bytes())
        .unwrap();
...
fn main() {
    ...
    let commit_and_open_at =
        |coeffs: &[FieldElement<_>]| -> (Polynomial<_>, G1Point, FieldElement<_>) {
    ...
    let (p1, p1_comm, y1) = commit_and_open_at(&p1_coeffs);

    let (p2, p2_comm, y2) = commit_and_open_at(&p2_coeffs);

    let (p3, p3_comm, y3) = commit_and_open_at(&p3_coeffs);

    let (p4, p4_comm, y4) = commit_and_open_at(&p4_coeffs);
